...
class OrderRepository:
    ...
    async def get_all(self) -> Sequence[Orders]:
        result = await self.session.scalars(select(Orders).options(joinedload(Orders.offer)))
        return result.all()


    async def get_by_unique_code(self, unique_code: Orders.unique_code) -> Orders:
        result = await self.session.execute(
            select(Orders).options(joinedload(Orders.offer)).where(Orders.unique_code == unique_code)
        )
